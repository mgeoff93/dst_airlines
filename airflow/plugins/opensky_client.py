        ...
        while attempt < max_retries:
            try:
                ...
                if response.status_code == 429:
                    logging.warning("OpenSky quota exceeded")
                    return None
                ...
                if response.status_code == 503:
                    ...
                    logging.warning(f"503 error - retry {attempt}/{max_retries} in {wait_time}s")
                ...
                attempt += 1
                wait_time = backoff_factor ** attempt
                logging.error(f"Request error {attempt}/{max_retries}: {e}")
                time.sleep(wait_time)
