...
class MLClient:
	...
	def train_and_log_model(self, file_path: str):
		...
		with mlflow.start_run(run_name="Champion_Challenger_Run") as run:
			...
			metrics = {"R2_Score": r2_score(y_test, y_pred), "MAE": mean_absolute_error(y_test, y_pred)}
			...
			# Recupere le Champion actuel pour comparaison
			client = MlflowClient()
			try:
				...
				c_r2, c_mae, c_lat = prod_run.get("R2_Score", -1), prod_run.get("MAE", 0), prod_run.get("inference_latency_ms", 0)
			except:
				c_r2, c_mae, c_lat = -1, 0, 0

			# MAJ des métriques Prometheus
			self.metric_r2_score.labels(status='production').set(c_r2 if c_r2 != -1 else 0)
			self.metric_mae.labels(status='production').set(c_mae)
			self.metric_inference_speed.labels(status='production').set(c_lat)
			...
			top_features = [X.columns[i] for i in np.argsort(importances)[-3:]]
			
			promoted = metrics["R2_Score"] > c_r2 and latence_ms < 200 and "departure_difference" in top_features
			
			if promoted:
				logging.info("PROMOTION VALIDEE : Le challenger devient la production")
